...
add_action('init', 'insert_ks_data');

function insert_ks_data() {
	add_filter('id_funds_raised', 'update_ks_raised', 1, 2);
	add_filter('id_number_pledges', 'update_ks_pledges', 1, 2);
	add_filter('id_percentage_raised', 'update_ks_percentage', 1, 4);
}
	...
	if (!empty($ks_raised)) {
		if ($goal > 0) {
			...
			if ($project_id > 0) {
				...
				$new_raised = floatval($raised + $ks_raised);
				if ($new_raised > 0) {
					$percentage = ($new_raised / $goal) * 100;
				}
				else {
					$percentage = '0';
				}
