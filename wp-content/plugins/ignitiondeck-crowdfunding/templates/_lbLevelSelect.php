							...
							<select name="level_select" class="idc-dropdown__select level_select">
								<?php foreach ($level_data as $level) {
									if (empty($level->level_invalid) || !$level->level_invalid) {
										echo '<option value="'.$level->id.'" data-price="'.(isset($level->meta_price) ? $level->meta_price : '').'" data-desc="'.$level->meta_short_desc.'" '.apply_filters('idcf_dropdown_option_attributes', '', $level).'>'.$level->meta_title.'</option>';
									}
								}
								?>
							</select>
